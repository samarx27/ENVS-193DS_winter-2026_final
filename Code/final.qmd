...
## a. 
We determined that there is not enough evidence to suggest a difference between the median flooded wetland area across water year classification (wet, above normal, below normal, dry, critical drought).
In part 1, th
...
cohens_d(wake_up_time_min ~ school_day_yes_no, #formula: response variable ~ grouping variable
  ...
  var.equal = TRUE) #argument for equal variances
```
There was a significant difference in mean wake up time between school (n=25) and non school days (n=11) with a large effect size (Student's two sample t-test, t(4.6445) = 34, p<0.001, $\alpha$ = 0.05, Cohen's d = 1.64). 
...
ggplot(data = my_data_summary, # use the summary data frame
  ...
  labs(title = "Mean and Standard Error for Wake up times on school and non school days", #providing title
       x = "Type", #naming x axis
       y = "Mean and SE million metric tons production") + #naming y axis
