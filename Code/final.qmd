...
slice_sample(kelp_clean, #taking a sample of kelp_clean
             n = 5) #displaying 5 random rows
str(kelp_clean) #inspecting the structure of kelp_clean
```
...
#| warning: false
#| include: false
mopl_clean <- mopl |> #creating new data frame 
  clean_names() |> #cleaning the names 
  mutate(used_bin = case_when(
    id == "used" ~ 1, #changing the used variable to a 1 
    id == "available" ~ 0)) |> #changing the available variable to zero
  select(used_bin, edgedist, vo_nest) #selecting three columns
...
AICc( #running the AICc for each of the models 
  ...
  arrange(AICc) #arranging AICcs from lowest to highest 
```
...
model_preds #displaying output
```

```{r}
ggplot(data = model2, #using model2 data frame
  ...
  labs(x = "Visual Obstruction", #labeling x axis
       y = "Presence of Prairie Dog Nesting Site") #labeling y axis 
```
...
predict_prob_vo0 #displaying output
```
...
my_data_clean <- my_data |> 
  clean_names() #cleaned names
ggplot(data = my_data_clean, #creating new data frame
       aes(sample = wake_up_time_hrs_min)) + #using the sample from the data frame
  geom_qq() + #adding qq points 
  geom_qq_line() + #adding qq line
  facet_wrap(~ school_day_yes_no) #adding it based on the two cateogories 
```
...
I completed the activity in section this week.
